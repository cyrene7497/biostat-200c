...
```{r}
pima1 <- pima1 %>%
  mutate(insulin.na = insulin) 
pima1$insulin.na[pima1$insulin == 0] <- NA
ggplot(pima1, aes(x = insulin.na, fill = test_f2)) +
  geom_histogram(alpha =.5, position = "identity") +
  ylab("Count") + xlab("Insulin (mu U/ml)") +
  ggtitle("Histogram of Insulin by Diabetes Test Results") +
  theme_bw()
...
If not associated then it's missing associated with randomness. Then try to reserve data and do 3-5 replicates of imputations? but bc missing vector that we created is not associated with the outcome you can use complete case analysis for this problem. This leads to a smaller data set that we're workign with. It will not give you the power for your test. due to smaller sample size after deleting missing. but the effect size is still valid. bc missingness is not associated with outcome. if you get rid of the missing it still accurately reflects if you didn't delete those outcomes. refit w na.omit
### Q3.7
...
add BMI back in if its gone and then refer to lab. 
