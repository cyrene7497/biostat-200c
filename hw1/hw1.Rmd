...
```{r}
biglm <- lm(Fertility ~ (Agriculture + Examination + Education + Catholic + Infant.Mortality)^2, swiss1)
smalllm <- step(biglm, trace = TRUE)
drop1(smalllm, test = "F")
```
The F-test shows that the `Agriculture:Examination` and `Agriculture:Education` interaction terms can be dropped from the model. 

So the final model is:
```{r}
# final model
finallm <- lm(Fertility ~ Agriculture + Examination + Education + Catholic + 
                Infant.Mortality + Agriculture:Infant.Mortality + 
                Examination:Education + Examination:Infant.Mortality + 
                Education:Catholic, swiss1)
summary(finallm)
finallm %>%
  tbl_regression() %>%
  bold_labels() %>%
