...
### The swiss data using Fertility as the response variable
```{r}
sessionInfo()
```
```{r}
...
The previous scatterplot of `Infant.Mortality` and `Fertility` indicated that it might be helpful to transform `Infant.Mortality` down the ladder of powers. 
```{r}
ggplot(data = swiss1) + 
    geom_point(mapping = aes(x = Infant.Mortality^(-.5), y = Fertility))
```
The transformation didn't improve the apparent fit much.
```{r}
swiss1 <- swiss1 %>%
  mutate(TInfant.M = Infant.Mortality^(-.5)) %>%
  print(width = Inf)
trlm <- lm(Fertility ~ Agriculture + Examination + Education + 
             Catholic2 + TInfant.M, swiss1)
crPlots(trlm)
```
...
#   filter(cook >= 0.1) %>%
#   print(width = Inf)
cooks.distance(lmodf)
```
None of the cook's distances are high, all are lower than 0.1, so 
```{r}
swiss1 %>%
  slice(c(1, 6, 19, 37, 46, 47)) %>% print(width = Inf)
```
