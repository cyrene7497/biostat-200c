...
```
We see that there are more negative residuals above the diagonal, while those below the diagonal are mostly positive. So there is generally more unskilled in '71 who moved up by '81 than the reverse. We can go on to check **marginal homogeneity**:
```{r}
margin.table(tab, 1)
margin.table(tab, 2)
```
