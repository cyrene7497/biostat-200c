...
```{r}
library(sandwich)
glm(Species ~ . - Endemics, family = poisson, data = gala) %>%
  vcovHC() %>%
  diag() %>%
  sqrt()
```

The standard errors of the regression coefficients are quite similar to the previous model with the dispersion parameter.
...
```{r}
library(robust)
rbm <- glmRob(Species ~ . - Endemics, family = poisson, gala)
summary(rbm)
```
