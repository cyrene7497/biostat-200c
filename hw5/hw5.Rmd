...
+ $\widehat{\sigma}_{\alpha}^2 &=& \frac{\text{SSA}/(a-1) - \widehat{\sigma}_{\epsilon}^2}{n}$
\begin{eqnarray*}
\mathbb{E}(SST) &=& \mathbb{E}\left[ \sum^a \sum^n (y_{ij} - \bar{y}_{\cdot \cdot})^2 \right] \\
...
&=& n(a-1) \sigma\{\alpha}^2 + na\sigma_{\epsilon}^2 - \sigma_{\epsilon}^2 - na\sigma_{\epsilon}^2 + a\sigma_{\epsilon}^2 \\
&=& n(a-1) \sigma\{\alpha}^2 + (a-1)\sigma_{\epsilon}^2 \\
&=& (a-1)(n\sigma\{\alpha}^2 + \sigma_{\epsilon}^2)
\end{eqnarray*}
...
\begin{eqnarray*}
L(\mu, \sigma_\alpha^2, \sigma_\epsilon^2) &=& \prod_{i=1}^a \prod_{j=1}^n \frac{1}{\sqrt{2\pi(\sigma_\alpha^2 + \sigma_epsilon^2)}}\exp{\frac{1}{2(\sigma_\slpha^2 + \sigma_epsilon^2)}(y_{ij} - \mu)^2} \\
\ell(\mu, \sigma_\alpha^2, \sigma_\epsilon^2) &=& \sum^a \sum^n -\frac{1}{2}\log2\pi - \frac{1}{2}\log(\sigma_\alpha^2 + \sigma_epsilon^2) - \frac{1}{2(\sigma_\alpha^2 + \sigma_epsilon^2)}(y_{ij} - \mu)^2 \\
\frac{d\ell}{d\mu} &=& - \sum^a \sum^n \frac{1}{\sigma_\alpha^2 + \sigma_epsilon^2} (y_{ij} - \mu) \\
\Rightarrow 0 &=& -\frac{1}{\sigma_\alpha^2 + \sigma_epsilon^2} \sum^a \sum^n(y_{ij} - \widehat{\mu}) \\
